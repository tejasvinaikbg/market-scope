...
export interface FileIssue { row?: number; column?: string; message: string }
...
export interface PortfolioRowInput {
  rowNumber: number;
  storeName: string; address: string; city: string; state: string; country: string; category: string;
  latitude: number | null; longitude: number | null;
}

/** BOM off, trimmed, lower-cased, runs of spaces/dashes → one underscore. */
export const normaliseHeader = (raw: string): string => raw.replace(/^﻿/, '').trim().toLowerCase().replace(/[\s-]+/g, '_');
...
export function validateHeaders(rawHeaders: string[]): HeaderCheck {
  ...
  rawHeaders.forEach((raw, i) => {
    ...
    if (seen.has(name)) { errors.push({ column: name, message: `duplicate column "${name}"` }); return; }
    seen.add(name);
  ...
  const hasLat = columnIndex.latitude !== undefined, hasLng = columnIndex.longitude !== undefined;
...
export function parseRow(cells: string[], columnIndex: HeaderCheck['columnIndex'], rowNumber: number): { row: PortfolioRowInput | null; issues: FileIssue[] } {
  ...
  const get = (col: KnownColumn): string => { const i = columnIndex[col]; return i === undefined ? '' : (cells[i] ?? '').trim(); };
  ...
  const latRaw = get('latitude'), lngRaw = get('longitude');
  ...
  if (latRaw !== '' || lngRaw !== '') {
    if (latRaw === '' || lngRaw === '') {
      ...
      latitude = Number(latRaw); longitude = Number(lngRaw);                       // Number('12abc') is NaN → caught below
      if (!Number.isFinite(latitude) || Math.abs(latitude) > 90) issues.push({ row: rowNumber, column: 'latitude', message: `latitude "${latRaw}" must be a number between -90 and 90` });
      if (!Number.isFinite(longitude) || Math.abs(longitude) > 180) issues.push({ row: rowNumber, column: 'longitude', message: `longitude "${lngRaw}" must be a number between -180 and 180` });
    }
  }

  if (issues.length) return { row: null, issues };
  return {
    row: { rowNumber, storeName: get('store_name'), address: get('address'), city: get('city'), state: get('state'), country: get('country'), category: get('category'), latitude, longitude },
    issues,
  };
}