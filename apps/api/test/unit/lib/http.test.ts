...
test('identifies the app, asks for JSON, and returns a 2xx response', async () => {
  ...
  const request = createThrottledFetch({ userAgent: 'market-scope/test', fetchImpl: fake((_url, init) => { seen = init; return Response.json({ ok: 1 }); }) });
...
test('429, 5xx and network errors are retryable; other failures are final and carry the body', async () => {
  const codes = async (status: number) => { try { await createThrottledFetch({ userAgent: 't', fetchImpl: fake(() => new Response('body', { status })) })('svc', 'https://x/'); } catch (err) { return err as Error; } };
  ...
  const network = createThrottledFetch({ userAgent: 't', fetchImpl: fake(() => { throw new Error('ECONNRESET'); }) });
...
test('a 406 says the User-Agent was rejected; an HTML error body becomes its title', async () => {
  const at = (status: number, body: string) => createThrottledFetch({ userAgent: 'x/1 (you@example.com)', fetchImpl: fake(() => new Response(body, { status })) })('overpass', 'https://x/');
...
test('withRetry retries only retryable errors, passing the attempt number', async () => {
  ...
  const out = await withRetry(async (attempt) => { attempts.push(attempt); if (attempt < 3) throw retryable('busy'); return 'done'; }, { retries: 3, minTimeout: 1 });
  assert.deepEqual([out, attempts], ['done', [1, 2, 3]]);
  let calls = 0;
  await assert.rejects(withRetry(async () => { calls++; throw new Error('final'); }, { retries: 3, minTimeout: 1 }), /final/);
