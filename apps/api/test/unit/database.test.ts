...
test('an explicit TEST_DATABASE_URL wins, wherever it points', () => {
  assert.equal(
    testDatabaseUrl({ DATABASE_URL: 'postgres://u:p@localhost:5432/ms', TEST_DATABASE_URL: 'postgres://u:p@db:5433/other' }),
    'postgres://u:p@db:5433/other',
  );
});

test('the dev database itself is refused, and so is no DATABASE_URL at all', () => {
  assert.throws(
    () => testDatabaseUrl({ DATABASE_URL: 'postgres://u:p@localhost:5432/ms', TEST_DATABASE_URL: 'postgres://u:p@localhost:5432/ms' }),
    /must not run against the dev database \(ms\)/,
  );
