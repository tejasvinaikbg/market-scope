...
const run = (err: unknown) => {
  ...
  (errorHandler as (...args: unknown[]) => void)(err, req, res, () => {});
  return out;
};

test('an AppError maps to its own status and code', () => {
  assert.deepEqual(run(createAppError(404, 'NOT_FOUND', 'city not found')), {
    status: 404,
    body: { error: { code: 'NOT_FOUND', message: 'city not found', details: undefined } },
  });
});
...
test('notFoundHandler answers with the JSON envelope', () => {
  const { res, out } = fakeRes();
  (notFoundHandler as (...args: unknown[]) => void)({}, res, () => {});
