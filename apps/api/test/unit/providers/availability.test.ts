...
const defaults = { OVERPASS_ENABLED: true, NOMINATIM_ENABLED: true, GOOGLE_PLACES_API_KEY: undefined, GOOGLE_PLACES_ENABLED: true, GOOGLE_GEOCODING_API_KEY: undefined, GOOGLE_GEOCODING_ENABLED: true };
...
test('by default the OSM providers are on and Google is "not configured"', () => {
  ...
  assert.deepEqual(p.places.map((o) => [o.id, o.enabled, o.reason]), [['overpass', true, null], ['google', false, 'not configured']]);
  assert.deepEqual(p.geocoding.map((o) => [o.id, o.enabled, o.reason]), [['nominatim', true, null], ['google', false, 'not configured']]);
});
...
test('a key enables a Google provider; every switch disables its own provider and no other', () => {
  ...
  assert.deepEqual(overpassOff.places.map((o) => [o.id, o.enabled]), [['overpass', false], ['google', true]]);
  const nominatimOff = listProviders({ ...defaults, NOMINATIM_ENABLED: false });
  assert.deepEqual(nominatimOff.geocoding.map((o) => o.reason), ['disabled', 'not configured']);   // both off: the screen can offer nothing
});
