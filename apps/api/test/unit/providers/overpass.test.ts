...
const categories = [
  ...
  { categoryId: 2, slug: 'pharmacy', selectors: [{ key: 'amenity', value: 'pharmacy' }, { key: 'shop', value: 'chemist' }] },
];
...
test('the query has a node and a way clause per selector, inside the tile, and asks for centres and tags', () => {
  ...
  assert.ok(buildOverpassQuery(tile, [{ categoryId: 9, slug: 'x', selectors: [{ key: 'name', value: 'Joe\'s "Mart" \\ Co' }] }]).includes('"Joe\'s \\"Mart\\" \\\\ Co"'));
});

test('toPlace: ways use their centre, a brand stands in for a name, unnamed shops are still shops, non-matches are dropped', () => {
  const way = toPlace({ type: 'way', id: 7, center: { lat: 12.91, lon: 77.61 }, tags: { shop: 'supermarket', brand: 'More', 'addr:street': '24th Main' } }, categories);
...
test('a 429 is retried on the next mirror; the answer is parsed', async () => {
  ...
  assert.deepEqual(out.map((p) => p.name), ['X']);
});

test('a 400 is final; a timed-out remark is transient; no categories means no request', async () => {
  let calls = 0;
  const bad = createOverpassProvider({ userAgent: 't', endpoints: ['https://a/'], fetchImpl: (async () => { calls++; return new Response('syntax error', { status: 400 }); }) as unknown as typeof fetch });
  await assert.rejects(bad.discover(tile, categories), /overpass 400/);
  assert.equal(calls, 1);

  let n = 0;
  const slow = createOverpassProvider({ userAgent: 't', endpoints: ['https://a/'], fetchImpl: (async () => { n++; return Response.json(n === 1 ? { remark: 'runtime error: Query timed out in "query"' } : { elements: [] }); }) as unknown as typeof fetch });
  assert.deepEqual(await slow.discover(tile, categories), []);
  assert.equal(n, 2);

  const never = createOverpassProvider({ userAgent: 't', fetchImpl: (async () => { throw new Error('must not be called'); }) as unknown as typeof fetch });
