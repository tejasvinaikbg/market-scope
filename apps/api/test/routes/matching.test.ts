...
const moveStore = (name: string, lat: number, lng: number) =>
  db('portfolio_stores')
    .where({ portfolio_id: portfolioId, store_name: name })
    .update({ location: db.raw('ST_SetSRID(ST_MakePoint(?, ?), 4326)::geography', [lng, lat]) });
...
test('the matched filter selects the pairs, beside the other layers, never instead of them', async () => {
  ...
  assert.deepEqual(
    only.stores.map((s: { id: string; name: string }) => [s.id[0], s.name]),
    [['p', 'FreshMart Koramangala']],
  );
