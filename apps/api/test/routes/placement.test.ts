...
test('before geocoding: the seven uploaded points are judged, the three without a point are unlocated', async () => {
  ...
  const inside = await db('market_portfolio_stores as p').join('portfolio_stores as s', 's.id', 'p.portfolio_store_id').where({ 'p.market_id': marketId, placement: 'inside' }).select('s.store_name');
  assert.deepEqual(inside.map((r) => r.store_name), ['FreshMart Koramangala']);
});
...
test('a point exactly on the edge counts as inside (ST_Covers, not ST_Contains)', async () => {
  ...
  await db('portfolio_stores').where({ id }).update({ location: db.raw('ST_SetSRID(ST_MakePoint(77.60, 12.92), 4326)::geography') });   // the south-west corner
