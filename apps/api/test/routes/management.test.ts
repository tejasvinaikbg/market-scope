...
const settle = async (id: number) => {
  ...
  await db('jobs').where({ market_id: id, status: 'pending' }).update({ status: 'done' });
};
...
test('a finished market can be run again: queued afresh, its rows kept until the run rewrites them, one more job in the queue', async () => {
  ...
  assert.deepEqual(
    [(await market(m.id)).status, (await market(m.id)).storeCount, (await market(m.id)).matched, (await market(m.id)).busy],
    ['ready', 4, 1, false],
  );
...
test('a finished market can be edited in place: new decisions stored, what was found discarded, queued afresh', async () => {
  ...
  assert.deepEqual(
    [edited.status, edited.storeCount, edited.matched, edited.busy, edited.categories.map((c: { slug: string }) => c.slug), edited.boundary.south],
    ['pending', 0, 0, true, ['pharmacy'], 12.93],
  );
  ...
  const bad = await fetch(`${base}/api/markets/${m.id}`, {
    method: 'PUT',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ portfolioId, cityId: 1, categoryIds: [1], boundary: { south: 12.8, west: 77.4, north: 13.2, east: 77.9 } }),
  });
