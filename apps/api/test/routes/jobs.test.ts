...
after(async () => {
  if (created.length) await db('jobs').whereIn('id', created).del();
  await db.destroy();
});
const enqueue = async (type: string) => {
  const id = await jobsQueries.enqueue(type, null, { hello: 'world' });
  created.push(id);
  return id;
};
...
test('a job left running by a dead worker is taken over once it is stale', async () => {
  ...
  await db('jobs')
    .where({ id })
    .update({ status: 'running', attempts: 1, updated_at: db.raw("now() - interval '20 minutes'") });
