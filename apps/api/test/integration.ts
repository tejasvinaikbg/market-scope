 ...
 * DATABASE_URL pointing at it, the worker off, the fixture providers on, no Google keys (so the suites see the same
 * availability on every machine), and the suites one file at a time, because they share that database. Exits with the
 * runner's own status.
 */
