...
export function createJobRunner(handlers: Record<string, JobHandler>, opts: RunnerOptions = {}) {
  ...
  async function tick(): Promise<void> {
    if (busy) return;
    busy = true;
    try { while (await runOnce()) { /* keep draining */ } }
    catch (err) { log('job runner error', { error: (err as Error).message }); }   // the queue itself failed (e.g. database down); the next tick retries
    finally { busy = false; }
  }

  return {
    runOnce,
    start() { if (!timer) { timer = setInterval(() => { void tick(); }, pollMs); void tick(); } },
