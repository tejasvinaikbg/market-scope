...
export interface CityRow { id: number; name: string; state: string; country: string; bbox: Bbox | null; centre: LatLng | null }
...
export const citiesQueries = {
  ...
  async byId(id: number, k: Db = db): Promise<CityRow | null> {
    const r = await k('cities as ci')
      ...
      .select(
        'ci.id', 'ci.name', 's.name as state', 'c.name as country',
        // bbox/centre are geometry columns: read them back as plain numbers
        k.raw('ST_YMin(ci.bbox) AS south, ST_XMin(ci.bbox) AS west, ST_YMax(ci.bbox) AS north, ST_XMax(ci.bbox) AS east, ST_Y(ci.centre) AS lat, ST_X(ci.centre) AS lng'),
      )
      .where('ci.id', id)
      .first();
    if (!r) return null;
  ...
  async cacheBounds(id: number, bbox: Bbox, centre: LatLng, k: Db = db): Promise<void> {
    await k('cities').where({ id }).update({
