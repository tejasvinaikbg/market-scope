...
import {
  ...
  squareAround,
  padBbox,
  MAX_MARKET_AREA_SQ_KM,
  DEFAULT_MARKET_AREA_SQ_KM,
  type Bbox,
} from '@market-scope/shared';
...
function SetupScreen() {
  ...
  // Seed the form from the market being edited, once its row and the locations tree are both here. Done during render
  // and guarded by the market's id, the way React asks for state that follows data, rather than in an effect.
  const [seededFor, setSeededFor] = useState<number | null>(null);
  if (editing.data && locations.data && seededFor !== editing.data.id) {
    const m = editing.data;
    const home = locations.data.countries
      .flatMap((c) => c.states.map((s) => ({ country: c, state: s })))
      .find(({ state }) => state.cities.some((city) => city.id === m.cityId));
    if (home) {
      setSeededFor(m.id);
      setCountryId(home.country.id);
      setStateId(home.state.id);
      setCityId(m.cityId);
      setSelected(m.categories.map((c) => c.id));
      setPlaces(m.placesProvider);
      setGeocoder(m.geocoderProvider);
  ...
  const setBoundary = (box: Bbox) => {
    if (cityId) setDraft({ cityId, box });
  };
  ...
  const onCreate = () => {
    ...
    if (editId) update.mutate({ id: editId, input }, toDashboard);
    else create.mutate(input, toDashboard);
  };
      ...
      <section className="space-y-6 border-b border-line bg-surface p-4 md:overflow-y-auto md:border-b-0 md:border-r md:p-6">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-bold">
            <Crosshair size={22} /> {editId ? 'Edit the market' : 'Define the market'}
          </h1>
          <p className="mt-1 text-muted">
            {editId
              ? 'Saving runs discovery again; what was found for the old boundary and categories is replaced.'
              : 'Boundary and categories decide how much store discovery costs.'}
          </p>
          {editId && editing.isError && (
            <div role="alert" className="mt-2 border border-bad bg-surface p-3 text-sm font-medium text-bad">
        ...
        {(locations.isError || categories.isError || providers.isError) && (
          <div role="alert" className="border border-bad bg-surface p-3 text-sm font-medium text-bad">
        ...
        >
          ...
          {locations.data?.countries.map((c) => (
        ...
        {cityId && (
          <div>
            <div className="caption mb-2 flex items-center gap-1.5">
              <ShoppingBag size={14} /> Categories
            </div>
        ...
        {selected.length > 0 && (
          <div className="space-y-3 border-t border-line pt-5">
            ...
            <Field label="Address lookup" value={geocoder} onChange={(e) => setGeocoder(e.target.value as 'nominatim' | 'google')}>
              {providers.data?.geocoding.map((p) => (
                <option key={p.id} value={p.id} disabled={!p.enabled}>
                  {p.name}
                  {p.reason ? ` · ${p.reason}` : ''}
                </option>
            ...
            {providers.data && !sourcesAvailable && (
              <p className="text-sm font-medium text-bad" role="alert">
                No data source is available for one of these. Ask whoever runs the service to enable one.
              </p>
            )}
        ...
        {city && boundary && (
          <div className="space-y-2 border-t border-line pt-5">
            <div className="caption flex items-center gap-1.5">
              <Maximize2 size={14} /> Boundary area
            </div>
            ...
            <div className="h-1 w-full bg-line">
              <div className={`h-full ${over ? 'bg-bad' : 'bg-accent'}`} style={{ width: `${Math.min(100, (area / MAX_MARKET_AREA_SQ_KM) * 100)}%` }} />
            </div>
            {dims && (
              <p className="text-xs text-muted">
                {dims.widthKm.toFixed(1)} × {dims.heightKm.toFixed(1)} km · ≈ {calls} calls
              </p>
            )}
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              {summary?.bounds && (
                <button
                  type="button"
                  onClick={() => setBoundary(padBbox(summary.bounds!, 0.5))}
            ...
            >
              {saving.isPending ? (
                <span className="flex items-center gap-2">
                  <Loader size={16} className="animate-spin" /> {editId ? 'Saving…' : 'Creating…'}
                </span>
              ) : portfolioId == null ? (
