            ...
            {markets.data.map((m) => {
              ...
              return (
                <li key={m.id} className="flex items-stretch">
                  <Link href={`/dashboard/${m.id}`} className="flex min-w-0 flex-1 flex-wrap items-center justify-between gap-x-6 gap-y-1 px-4 py-3 hover:bg-panel">
                  ...
                  {!inFlight && confirmId !== m.id && (
                    <>
                      <Link href={`/setup?market=${m.id}`} aria-label={`Edit ${m.name}`} title="Edit this market" className="flex items-center px-3 text-muted hover:bg-panel hover:text-fg"><Pencil size={16} /></Link>
                      <button type="button" aria-label={`Delete ${m.name}`} title="Delete this market" onClick={() => setConfirmId(m.id)}
                        className="flex items-center px-3 text-muted hover:bg-panel hover:text-bad"><Trash2 size={16} /></button>
                    </>
                  )}
                  {confirmId === m.id && (
                    <div role="alertdialog" aria-label={`Delete ${m.name}?`} className="flex flex-wrap items-center gap-2 border-l border-line px-3 py-2 text-sm">
                      <span>Delete, with everything found for it?</span>
                      <button type="button" disabled={remove.isPending} onClick={() => remove.mutate(m.id, { onSuccess: () => setConfirmId(null) })}
                        className="rounded bg-bad px-3 py-1 font-semibold text-white disabled:opacity-60">{remove.isPending ? 'Deleting…' : 'Delete'}</button>
                      <button type="button" onClick={() => setConfirmId(null)} className="rounded border border-line px-3 py-1 font-medium">Keep it</button>
