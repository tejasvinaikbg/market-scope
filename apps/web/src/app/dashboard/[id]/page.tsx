...
const located = (g: Market['geocoding']) =>
  g.total === 0 ? '' : ` ${g.done} of ${g.total} of your stores located from their address${g.failed ? `, ${g.failed} not found` : ''}.`;
const placed = (p: Market['placement']) => {
  const total = p.inside + p.outside + p.unlocated;
  return total === 0
    ? ''
    : ` ${p.inside} of your ${total} stores inside the boundary, ${p.outside} outside${p.unlocated ? `, ${p.unlocated} without a location` : ''}.`;
};
  ...
  switch (m.status) {
    case 'pending':
      return (
        <p className="flex items-center gap-2 text-muted">
          <Loader size={16} className="animate-spin" /> Discovery is queued and starts in a moment.
        </p>
      );
    case 'running':
      ...
      if (!p && g.total > 0) {
        return (
          <p className="flex items-center gap-2">
            <Loader size={16} className="animate-spin text-accent" /> Locating your stores from their addresses… {g.done + g.failed} of {g.total}
          </p>
        );
      }
      return (
        <div className="space-y-2">
          <p className="flex items-center gap-2">
            <Loader size={16} className="animate-spin text-accent" /> Discovering stores… {p ? `${p.done} of ${p.tiles} areas searched` : 'starting'} ·{' '}
            {m.storeCount} found so far
          </p>
    ...
    case 'ready':
      return (
        <p className="flex items-center gap-2 text-ok">
          <Check size={16} /> {m.storeCount} stores discovered{p ? ` across ${p.tiles} areas` : ''}.{located(g)}
          {placed(m.placement)}
          {matched(m.matched)}
        </p>
      );
      ...
      return (
        <div className="space-y-1">
          <p className="flex items-center gap-2 text-bad">
            <TriangleAlert size={16} /> {m.storeCount} stores discovered; {p?.failed ?? 'some'} of {p?.tiles ?? 'the'} areas could not be searched.
          </p>
    ...
    case 'failed':
      return (
        <div className="space-y-1">
          <p className="flex items-center gap-2 text-bad">
            <TriangleAlert size={16} /> Discovery failed. No area could be searched.
          </p>
          {m.error && <p className="text-xs text-muted">{m.error}</p>}
        </div>
      );
...
function Totals({ m }: { m: Market }) {
  const cells: [string, number][] = [
    ['Discovered', m.storeCount],
    ['Portfolio inside', m.placement.inside],
    ['Portfolio outside', m.placement.outside],
    ['Not located', m.placement.unlocated],
    ['Matched', m.matched],
  ];
  return (
    ...
    <dl className="grid grid-cols-3 gap-px border border-line bg-line">
      {cells.map(([label, n]) => (
        <div key={label} className="bg-surface p-3 last:col-span-2">
...
export default function Page() {
  ...
  useEffect(() => {
    if (selected?.from === 'map') rowRefs.current.get(selected.id)?.scrollIntoView({ block: 'nearest' });
  }, [selected]);
  ...
  if (market.isPending)
    return (
      <div className="flex items-center gap-2 p-6 text-muted">
        <Loader size={16} className="animate-spin" /> Loading the market…
      </div>
    );
  if (market.isError || !market.data) {
    return (
      <div role="alert" className="m-4 border border-bad bg-surface p-4 text-bad md:m-6">
      ...
      <section className="flex flex-col border-b border-line bg-surface md:overflow-y-auto md:border-b-0 md:border-r">
        <div className="space-y-4 p-4 md:p-6">
          <div>
            <div className="caption flex items-center gap-2">
              <Layers size={14} /> Market
            </div>
            <h1 className="mt-1 text-2xl font-bold">{m.name}</h1>
            <p className="mt-1 text-muted">
              {m.cityName} · {m.areaSqKm.toFixed(1)} km² · portfolio {m.portfolioName}
            </p>
          ...
          <div className="border border-line bg-panel p-4" role="status" aria-live="polite">
            <div className="caption mb-2 flex items-center gap-2">
              <Info size={14} /> Store discovery
            </div>
            <DiscoveryStatus m={m} />
            {!inFlight && (
              <div className="mt-3 flex flex-wrap items-center gap-3">
                ...
                >
                  <Pencil size={12} /> Edit this market
                </Link>
                {rerun.isError && (
                  <span role="alert" className="text-xs text-bad">
                    {isApiError(rerun.error) ? rerun.error.message : "Couldn't reach the service."}
                  </span>
                )}
              </div>
            )}
            {ended && m.storeCount === 0 && (
              <p className="mt-2 text-sm text-muted">
                Nothing was found for {m.categories.map((c) => c.name).join(', ')} inside this boundary. Try a larger boundary or other categories.
              </p>
            )}
          </div>
          {started && <Totals m={m} />}
          {anything && (
            <>
              <div>
                <div className="caption mb-2 flex items-center gap-1.5">
                  <Layers size={14} /> Layers
                </div>
                <div className="flex flex-wrap gap-2">
                  {LAYERS.map((l) => (
                    <Chip key={l.id} on={layers.includes(l.id)} onClick={() => setLayers((s) => toggleIn(s, l.id))}>
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {m.categories.map((c) => (
                    <Chip key={c.id} on={categories.includes(c.slug)} onClick={() => setCategories((s) => toggleIn(s, c.slug))}>
        ...
        {anything && (
          <div className="border-t border-line">
            {layers.length === 0 ? (
              <p className="p-4 text-muted md:px-6">No layers selected. Turn one on to see stores.</p>
            ) : rows.length === 0 && stores.data ? (
              <p className="p-4 text-muted md:px-6">No stores match these filters.</p>
            ) : (
              <ul>
                {rows.map((s) => (
                  <li key={s.id} className="border-b border-line">
                    ...
                    <button
                      type="button"
                      aria-current={picked(s.id) || undefined}
                      onClick={() => pick(s.id, 'list')}
                      ref={(el) => {
                    ...
                    >
                      ...
                      <span className="flex shrink-0 flex-col items-end gap-1 pt-1">
                        <span className="caption flex items-center gap-1.5">
                          <LayerSwatch layer={s.layer} /> {layerTag(s.layer)}
                        </span>
                        {s.match && (
                          <span className="caption flex max-w-48 items-center gap-1.5 text-map-match">
                            <LayerSwatch layer="matched" />{' '}
                            <span className="truncate">
                              Matched · {s.match.name} · {s.match.distanceM} m
                            </span>
                          </span>
                        )}
                        {yours.has(s.id) && (
                          <span className="caption flex max-w-48 items-center gap-1.5 text-map-match" title={`Yours: ${yours.get(s.id)!.join(', ')}`}>
                            <LayerSwatch layer="matched" /> <span className="truncate">Yours · {yours.get(s.id)!.join(', ')}</span>
                          </span>
                        )}
                      </span>
                    </button>
            ...
            {unlocated.length > 0 && (
              <div className="border-t border-line pb-2">
                <div className="caption flex items-center gap-1.5 px-4 pt-4 md:px-6">
                  <MapPin size={14} /> Not on the map · {unlocated.length}
                </div>
                <ul>
                  {unlocated.map((s) => (
                    <li key={s.id} className="flex items-start gap-3 px-4 py-2.5 md:px-6">
                      ...
                      <span className="min-w-0">
                        ...
                        <span className="block text-sm text-muted">
                          {detail(s.category, s.address)} · {unlocatedReason(s.reason)}
                        </span>
                      </span>
                    </li>
                  ))}
            ...
            <div className="caption flex items-center justify-between gap-3 border-t border-line px-4 py-3 md:px-6">
              <span>
                {rows.length} {noun(rows.length)} shown · {total} in this market
              </span>
              {filtered && (
                <button type="button" onClick={clear} className="flex items-center gap-1 text-fg">
        ...
        <div className="mt-auto space-y-3 p-4 md:p-6">
          <div className="flex flex-wrap gap-2">
            <Link href="/dashboard" className="inline-flex items-center gap-2 rounded border border-line px-4 py-2 font-semibold">
              <ChevronLeft size={16} /> All markets
            </Link>
            <Link href="/setup" className="inline-flex items-center gap-2 rounded border border-line px-4 py-2 font-semibold">
              Create another market <ArrowRight size={16} />
            </Link>
          </div>
          {!inFlight && !confirmDelete && (
            <button type="button" onClick={() => setConfirmDelete(true)} className="text-sm text-muted underline-offset-2 hover:underline">
              Delete this market
            </button>
          )}
          {confirmDelete && (
            <div className="flex flex-wrap items-center gap-3 border border-bad bg-surface p-3 text-sm" role="alertdialog" aria-label="Delete this market?">
              ...
              <button
                type="button"
                onClick={() => marketId && remove.mutate(marketId, { onSuccess: () => router.push('/dashboard') })}
                disabled={remove.isPending}
                className="rounded bg-bad px-3 py-1.5 font-semibold text-white disabled:opacity-60"
              >
                {remove.isPending ? 'Deleting…' : 'Delete'}
              </button>
              <button type="button" onClick={() => setConfirmDelete(false)} className="rounded border border-line px-3 py-1.5 font-medium">
                Keep it
              </button>
              {remove.isError && (
