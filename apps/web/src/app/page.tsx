    ...
    <div className="grid grid-cols-1 md:min-h-[calc(100vh-8rem)] md:grid-cols-[1fr_300px]">
      <section className="p-4 md:p-8">
        ...
        <p className="mt-2 max-w-xl text-muted">
          A CSV or XLSX of your own stores. Headers are validated before anything is accepted — nothing is stored until the file passes.
        </p>
        ...
        >
          <div className="caption flex items-center gap-2 text-accent">
            <Upload size={14} /> PF file
          </div>
        ...
        {upload.isError && !error && (
          <div className="mt-6 border border-bad bg-surface p-4 md:p-6 font-semibold text-bad" role="alert">
        ...
        {error && (
          <div className="mt-6 border border-bad bg-surface p-4 md:p-6" role="alert">
            <div className="flex items-center gap-2 font-semibold text-bad">
              <TriangleAlert size={16} /> {error.message} — nothing was stored
            </div>
            {issues.length > 0 && (
              <div className="mt-3">
        ...
        {upload.isSuccess && (
          <div className="mt-6 border border-line bg-surface p-4 md:p-6">
            <div className="caption flex items-center gap-2 text-ok">
              <Check size={14} /> Stored
            </div>
            <div className="mt-2 flex items-center gap-2 text-lg font-bold">
              <FileText size={18} /> {upload.data.name}
            </div>
            <p className="mt-1 text-muted">
              {upload.data.rowCount} stores · {upload.data.withCoords} with coordinates · {upload.data.withoutCoords} to locate from their address
            </p>
      ...
      <aside className="border-t border-line bg-panel p-4 md:border-l md:border-t-0 md:p-6">
        ...
        <p className="mt-3 text-xs text-muted">
          Types are checked too: <code>latitude</code> and <code>longitude</code> must parse as decimal degrees when present.
        </p>
      </aside>
    </div>
  );
