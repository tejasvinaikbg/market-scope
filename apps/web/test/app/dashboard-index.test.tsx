...
const market = (id: number, extra: Record<string, unknown>) => ({
  ...
  status: 'ready', error: null, startedAt: null, completedAt: null, storeCount: 71, progress: null, geocoding: { total: 0, done: 0, failed: 0 }, placement: { inside: 0, outside: 0, unlocated: 0 }, matched: 0, busy: false,
  ...extra,
});
...
test('a finished market can be deleted from its row, asked first; a running one cannot', async () => {
  ...
  const spy = jest.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
    ...
    if (init?.method === 'DELETE' && url.endsWith('/api/markets/7')) { listed = [two[0]]; return { ok: true, status: 204, statusText: '204', json: async () => undefined } as Response; }
