...
test('a header rejection shows the table and crosses exactly the missing columns', async () => {
  mockApi({
    'POST /api/portfolios': {
      status: 400, body: {
        error: {
          code: 'INVALID_HEADERS', message: 'Header row is invalid', details: [
...
test('a stored upload shows the counts, checks every rail row, and updates the stepper', async () => {
  mockApi({ 'POST /api/portfolios': { status: 201, body: { id: 1, name: 'stores', sourceFilename: 'stores.csv', rowCount: 10, createdAt: '2026-09-06T00:00:00Z', withCoords: 7, withoutCoords: 3, warnings: [] } } });
  const { container } = renderApp(<><Stepper /><Page /></>);
  await userEvent.upload(fileInput(container), csv());
