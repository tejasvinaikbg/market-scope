...
const categories = [
  { id: 1, slug: 'supermarket', name: 'Supermarket' },
  { id: 2, slug: 'pharmacy', name: 'Pharmacy' },
];
const wholeCity = {
  cityId: 1,
  name: 'Bengaluru',
  bbox: { south: 12.8335, west: 77.4599, north: 13.1426, east: 77.7841 },
  centre: { lat: 12.97, lng: 77.59 },
  cached: true,
};
const koramangala = { south: 12.92, west: 77.6, north: 12.956, east: 77.646 }; // ~20 km² of located stores
const providers = {
  places: [
    { id: 'overpass', name: 'OSM Overpass', enabled: true, reason: null },
    { id: 'google', name: 'Google Places API (New)', enabled: false, reason: 'not configured' },
  ],
  geocoding: [
    { id: 'nominatim', name: 'OSM Nominatim', enabled: true, reason: null },
    { id: 'google', name: 'Google Geocoding API', enabled: false, reason: 'disabled' },
  ],
};
...
test('blocks appear in order, and the CTA only once the form is complete', async () => {
  mockApi({
    'GET /api/locations': { body: locations },
    'GET /api/categories': { body: categories },
    'GET /api/providers': { body: providers },
    'GET /api/cities/1/bbox': { body: wholeCity },
  });
  renderApp(<Page />);
...
test('with a portfolio in the session, "fit" wraps its located stores', async () => {
  mockApi({
    ...
    'GET /api/portfolios/1': {
      body: { id: 1, name: 'sample', sourceFilename: 'sample.csv', rowCount: 10, createdAt: '', withCoords: 7, withoutCoords: 3, bounds: koramangala },
    },
  });
  renderApp(
    <>
...
test('"Create market" posts every decision and moves to the new market', async () => {
  const fetchSpy = mockApi({
    ...
    'POST /api/markets': { status: 201, body: market },
  });
  renderApp(
    <>
      <WithPortfolio />
...
test("the server's rejection is shown in its own words, and nothing moves", async () => {
  mockApi({
    ...
    'POST /api/markets': {
      status: 400,
      body: { error: { code: 'AREA_TOO_LARGE', message: 'Boundary is 30.12 km²; the cap is 30 km²', details: { areaSqKm: 30.12 } } },
    },
  });
  renderApp(
    <>
...
test('the default is the first available source; with none available the CTA stays off and says why', async () => {
  const osmOff = {
    places: [
      { id: 'overpass', name: 'OSM Overpass', enabled: false, reason: 'disabled' },
      { id: 'google', name: 'Google Places API (New)', enabled: true, reason: null },
    ],
    geocoding: [
      { id: 'nominatim', name: 'OSM Nominatim', enabled: false, reason: 'disabled' },
      { id: 'google', name: 'Google Geocoding API', enabled: false, reason: 'not configured' },
    ],
  };
  mockApi({
    'GET /api/locations': { body: locations },
    'GET /api/categories': { body: categories },
    'GET /api/providers': { body: osmOff },
    'GET /api/cities/1/bbox': { body: wholeCity },
    'GET /api/portfolios/1': { body: summary },
  });
  renderApp(
